...
def input(key):
    ...
    if key == 'left mouse up' and selected_cubie and drag_start:
        ...
        if face_normal == Vec3(1, 0, 0) or face_normal == Vec3(-1, 0, 0):
            ...
            if abs(dy) > abs(dx):
                rotate_x_layer(selected_cubie)

        elif face_normal == Vec3(0, 0, 1) or face_normal == Vec3(0, 0, -1):
            if abs(dx) > abs(dy):
                rotate_z_layer(selected_cubie)
...
def rotate_x_layer(cubie):
    ...
    rotation_pivot.animate_rotation(
...
def rotate_y_layer(cubie):
    global is_rotating
    is_rotating = True
    layer_y = round(cubie.y)
    layer_cubies = [c for c in cubies if round(c.y) == layer_y]
    rotation_pivot.rotation = (0, 0, 0)
    for c in layer_cubies:
        c.parent = rotation_pivot
    rotation_pivot.animate_rotation(
...
def rotate_z_layer(cubie):
    global is_rotating
    is_rotating = True
    layer_z = round(cubie.z)
    layer_cubies = [c for c in cubies if round(c.z) == layer_z]
    rotation_pivot.rotation = (0, 0, 0)
    for c in layer_cubies:
        c.parent = rotation_pivot
    rotation_pivot.animate_rotation(
